    ...
    final dio = Dio(
      BaseOptions(
        ...
        headers: {
          ...
          'User-Agent': 'Fridgee/1.0 (Flutter; contact@fridgee.app)',
        },
      ),
    );
...
/// Converts Dio exceptions into readable [FridgeeApiException]s.
class _ErrorInterceptor extends Interceptor {
  ...
  void onError(DioException err, ErrorInterceptorHandler handler) {
    final exception = switch (err.type) {
      ...
      DioExceptionType.connectionError => FridgeeApiException(
          message: 'Brak połączenia z internetem.',
          statusCode: null,
          original: err,
        ),
      DioExceptionType.badResponse => FridgeeApiException(
          message: 'Błąd serwera (${err.response?.statusCode}).',
          statusCode: err.response?.statusCode,
          original: err,
        ),
      _ => FridgeeApiException(
...
class FridgeeApiException implements Exception {
  const FridgeeApiException({
  ...
  @override
  String toString() => 'FridgeeApiException($statusCode): $message';
}
